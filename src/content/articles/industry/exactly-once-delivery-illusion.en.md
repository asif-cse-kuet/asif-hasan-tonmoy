> **Lesson 139 · advanced** - At-least-once plus idempotent consumers is the practical ceiling - and why brokers lie gently.
...
- This lesson is specifically about **The exactly-once delivery illusion**. Tags: messaging, exactly-once, idempotency.
...
flowchart TD
  ...
  C --> D["User complaint"]
  D --> E["The exactly-once delivery illusion"]
  E --> F["Idempotent consumer"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: At-least-once plus idempotent consumers is the practical ceiling - and why brokers lie gently.
...
At-least-once plus idempotent consumers is the practical ceiling - and why brokers lie gently. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Queue depth, dead-letter rate, and duplicate side-effect count. If the chart cannot catch a regression in **The exactly-once delivery illusion**, the lesson is not done.
...
- [ ] The invariant for **The exactly-once delivery illusion** is written in one sentence.
...
- [ ] Related reading still makes sense: backpressure-queue-design.
