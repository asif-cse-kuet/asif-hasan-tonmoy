> **পাঠ 155 · উন্নত ধাপ** - ছোট request চলে, বড় POST ঝুলে থাকে: PMTU black hole, tunnel, আর কেউ যাচাই না করা ১৫০০-byte ধারণা।
...
- এই পাঠটা ঠিক **MTU, MSS clamping ও নীরব packet loss** নিয়ে। ট্যাগ: mtu, mss, pmtud, tcp, packet-loss।
...
flowchart TD
  ...
  B --> C["অরিজিন PHP"]
  C --> D["৫০৪ বা হ্যাং"]
  D --> E["MTU, MSS clamping ও নীরব packet loss"]
  E --> F["বাজেট মেলানো"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: ছোট request চলে, বড় POST ঝুলে থাকে: PMTU black hole, tunnel, আর কেউ যাচাই না করা ১৫০০-byte ধারণা।
...
ছোট request চলে, বড় POST ঝুলে থাকে: PMTU black hole, tunnel, আর কেউ যাচাই না করা ১৫০০-byte ধারণা। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
এজ ৪xx/৫xx, TLS হ্যান্ডশেক সময়, আর অরিজিন vs এজ হিট। **MTU, MSS clamping ও নীরব packet loss**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **MTU, MSS clamping ও নীরব packet loss**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: tls-handshake-cost-and-resumption, websockets-through-proxies, nginx-config-debugging-playbook।
