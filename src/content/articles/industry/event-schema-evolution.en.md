> **Lesson 135 · advanced** - Compatibility modes, required-field traps, and rolling out a producer change when six consumers deploy on their own schedule.
...
- This lesson is specifically about **Event schema evolution without breaking consumers**. Tags: schema-registry, avro, compatibility, contracts, protobuf.
...
flowchart TD
  ...
  C --> D["User complaint"]
  D --> E["Event schema evolution without breaking consumers"]
  E --> F["Idempotent consumer"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Compatibility modes, required-field traps, and rolling out a producer change when six consumers deploy on their own schedule.
...
Compatibility modes, required-field traps, and rolling out a producer change when six consumers deploy on their own schedule. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Queue depth, dead-letter rate, and duplicate side-effect count. If the chart cannot catch a regression in **Event schema evolution without breaking consumers**, the lesson is not done.
...
- [ ] The invariant for **Event schema evolution without breaking consumers** is written in one sentence.
...
- [ ] Related reading still makes sense: poison-message-and-dlq, queue-vs-stream-selection, outbox-pattern-implementation.
