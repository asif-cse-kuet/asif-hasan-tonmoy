> **পাঠ 107 · মাঝারি ধাপ** - task-level retry, sensor storm, আর একটি step retry বনাম পুরো DAG run re-run-এর পার্থক্য।
...
- এই পাঠটা ঠিক **Amplify না করা pipeline orchestration retry** নিয়ে। ট্যাগ: airflow, orchestration, retries, sla, dag।
...
flowchart TD
  ...
  D --> E["Amplify না করা pipeline orchestration retry"]
  E --> F["চুক্তি + ওয়াটারমার্ক"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: task-level retry, sensor storm, আর একটি step retry বনাম পুরো DAG run re-run-এর পার্থক্য।
...
task-level retry, sensor storm, আর একটি step retry বনাম পুরো DAG run re-run-এর পার্থক্য। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
ফ্রেশনেস, ডুপ্লিকেট পার্টিশন রাইট, আর PII স্ক্যান। **Amplify না করা pipeline orchestration retry**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **Amplify না করা pipeline orchestration retry**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: idempotent-backfills, schema-drift-detection, batch-vs-streaming-pipelines।
