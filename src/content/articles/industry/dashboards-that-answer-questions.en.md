> **Lesson 71 · intermediate** - Ninety panels of CPU tell you nothing at 03:00. Start from the question, order panels by triage flow, and delete anything nobody has ever acted on.
...
- This lesson is specifically about **Dashboards that answer questions, not decorate walls**. Tags: grafana, dashboards, triage, promql, ux.
...
flowchart TD
  ...
  B --> C["Four log silos"]
  C --> D["Long TTD"]
  D --> E["Dashboards that answer questions, not decorate walls"]
  E --> F["Correlate + SLI"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Ninety panels of CPU tell you nothing at 03:00. Start from the question, order panels by triage flow, and delete anything nobody has ever acted on.
...
Ninety panels of CPU tell you nothing at 03:00. Start from the question, order panels by triage flow, and delete anything nobody has ever acted on. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
SLI burn, request-id join success, and alert pages that map to an owner. If the chart cannot catch a regression in **Dashboards that answer questions, not decorate walls**, the lesson is not done.
...
- [ ] The invariant for **Dashboards that answer questions, not decorate walls** is written in one sentence.
...
- [ ] Related reading still makes sense: red-and-use-methods, golden-signals-instrumentation, alert-design-and-noise-reduction.
