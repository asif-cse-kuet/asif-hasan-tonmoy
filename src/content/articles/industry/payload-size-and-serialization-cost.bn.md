> **পাঠ 67 · মাঝারি ধাপ** - JSON encoding query-র চেয়েও বেশি CPU খেতে পারে - response-প্রতি byte ও তার allocation মাপুন।
...
- এই পাঠটা ঠিক **Payload size ও serialisation-এর লুকানো খরচ** নিয়ে। ট্যাগ: serialization, json, payload, compression, bandwidth।
...
flowchart TD
  ...
  C --> D["টাইমআউট"]
  D --> E["Payload size ও serialisation-এর লুকানো খরচ"]
  E --> F["বাজেট + হিস্টোগ্রাম"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: JSON encoding query-র চেয়েও বেশি CPU খেতে পারে - response-প্রতি byte ও তার allocation মাপুন।
...
JSON encoding query-র চেয়েও বেশি CPU খেতে পারে - response-প্রতি byte ও তার allocation মাপুন। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
লেটেন্সির হিস্টোগ্রাম (শুধু গড় নয়), রেসপন্স বাইট, আর রিকোয়েস্টপ্রতি CPU। **Payload size ও serialisation-এর লুকানো খরচ**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **Payload size ও serialisation-এর লুকানো খরচ**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: batching-and-request-coalescing, cost-per-request-optimization, gc-pauses-and-memory-pressure।
