> **পাঠ 40 · মাঝারি ধাপ** - reconnect storm, missed event, server cursor-এ UI state reconcile।
...
- এই পাঠটা ঠিক **scale-এ WebSocket state sync** নিয়ে। ট্যাগ: websocket, realtime, frontend।
...
flowchart TD
  ...
  C --> D["মাইনর ভার্সন"]
  D --> E["scale-এ WebSocket state sync"]
  E --> F["কিবোর্ড বা LCP ইনসিডেন্ট"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: reconnect storm, missed event, server cursor-এ UI state reconcile।
...
reconnect storm, missed event, server cursor-এ UI state reconcile। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
প্রতি রুটে LCP, ডাউনলোড হওয়া JS, আর ডিপ্লয়প্রতি axe লঙ্ঘন। **scale-এ WebSocket state sync**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
