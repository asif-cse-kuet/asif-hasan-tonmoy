> **পাঠ 111 · মাঝারি ধাপ** - Paging authority ছাড়া ownership মানে একটা spreadsheet। Rotation size, page budget আর লেখা escalation path ঠিক করে senior engineer থাকবে না চুপচাপ চলে যাবে।
...
- এই পাঠটা ঠিক **On-call ও service ownership model** নিয়ে। ট্যাগ: on-call, ownership, incident-response, escalation, burnout।
...
flowchart TD
  ...
  D --> E["On-call ও service ownership model"]
  E --> F["ADR + কিল সুইচ"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: Paging authority ছাড়া ownership মানে একটা spreadsheet। Rotation size, page budget আর লেখা escalation path ঠিক করে senior engineer থাকবে না চুপচাপ চলে যাবে।
...
Paging authority ছাড়া ownership মানে একটা spreadsheet। Rotation size, page budget আর লেখা escalation path ঠিক করে senior engineer থাকবে না চুপচাপ চলে যাবে। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
ফ্ল্যাগ এক্সপোজার %, প্ল্যাটফর্ম অ্যাডপশন, আর টেন্যান্টপ্রতি খরচ। **On-call ও service ownership model**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **On-call ও service ownership model**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: internal-platform-as-product, technical-debt-prioritization, feature-flags-and-kill-switches।
