> **পাঠ 127 · উন্নত ধাপ** - shared schema, schema-per-tenant বা database-per-tenant - প্রতিটি পছন্দ cross-tenant leak-এর ঝুঁকি অন্য জায়গায় সরায়। tenant filter ORM-এর নিচে enforce করুন।
...
- এই পাঠটা ঠিক **Storage layer-এ multi-tenant data isolation** নিয়ে। ট্যাগ: multi-tenancy, rls, isolation, noisy-neighbour।
...
flowchart TD
  ...
  B --> C["পুল বা লক ওয়েট"]
  C --> D["টাইমআউট / ৫০০"]
  D --> E["Storage layer-এ multi-tenant data isolation"]
  E --> F["with() + কীসেট + ইনডেক্স"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: shared schema, schema-per-tenant বা database-per-tenant - প্রতিটি পছন্দ cross-tenant leak-এর ঝুঁকি অন্য জায়গায় সরায়। tenant filter ORM-এর নিচে enforce করুন।
...
shared schema, schema-per-tenant বা database-per-tenant - প্রতিটি পছন্দ cross-tenant leak-এর ঝুঁকি অন্য জায়গায় সরায়। tenant filter ORM-এর নিচে enforce করুন। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
রিকোয়েস্টপ্রতি কোয়েরি সংখ্যা, লিস্ট এন্ডপয়েন্টের p99, আর পুল ওয়েট টাইম। **Storage layer-এ multi-tenant data isolation**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **Storage layer-এ multi-tenant data isolation**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: sharding-key-selection, hot-partition-mitigation, soft-delete-and-audit-trails।
