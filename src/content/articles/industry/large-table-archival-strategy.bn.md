> **পাঠ 48 · মাঝারি ধাপ** - ১.৪ TB events table প্রতিটি backup, migration ও index rebuild ধীর করে। DELETE loop table আরও bloat করে; partition drop ভালো retention।
...
- এই পাঠটা ঠিক **বিশাল table archive ও prune করা** নিয়ে। ট্যাগ: retention, partitioning, vacuum, storage-cost।
...
flowchart TD
  ...
  B --> C["পুল বা লক ওয়েট"]
  C --> D["টাইমআউট / ৫০০"]
  D --> E["বিশাল table archive ও prune করা"]
  E --> F["with() + কীসেট + ইনডেক্স"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: ১.৪ TB events table প্রতিটি backup, migration ও index rebuild ধীর করে। DELETE loop table আরও bloat করে; partition drop ভালো retention।
...
১.৪ TB events table প্রতিটি backup, migration ও index rebuild ধীর করে। DELETE loop table আরও bloat করে; partition drop ভালো retention। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
রিকোয়েস্টপ্রতি কোয়েরি সংখ্যা, লিস্ট এন্ডপয়েন্টের p99, আর পুল ওয়েট টাইম। **বিশাল table archive ও prune করা**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **বিশাল table archive ও prune করা**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: zero-downtime-schema-migrations, index-design-and-query-plans, soft-delete-and-audit-trails।
