> **Lesson 44 · intermediate** - Append-only trails, actor attribution through async jobs, and retention that answers "who changed this?".
...
- This lesson is specifically about **Audit logging that survives compliance review**. Tags: audit, compliance, observability, retention.
...
flowchart TD
  ...
  D --> E["Audit logging that survives compliance review"]
  E --> F["Policy + cookie flags"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Append-only trails, actor attribution through async jobs, and retention that answers "who changed this?".
...
Append-only trails, actor attribution through async jobs, and retention that answers "who changed this?". Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Failed logins by reason, reset-token reuse, and cross-tenant 403 rate. If the chart cannot catch a regression in **Audit logging that survives compliance review**, the lesson is not done.
...
- [ ] The invariant for **Audit logging that survives compliance review** is written in one sentence.
...
- [ ] Related reading still makes sense: rbac-vs-abac-modeling, multi-tenant-authorization-leaks.
