> **পাঠ 87 · মাঝারি ধাপ** - কোন server block জিতল, regex location কেন match করল, রাত ২টায় $upstream_* variable কীভাবে পড়বেন।
...
- এই পাঠটা ঠিক **nginx config debugging playbook** নিয়ে। ট্যাগ: nginx, debugging, logging, playbook, edge।
...
flowchart TD
  ...
  B --> C["অরিজিন PHP"]
  C --> D["৫০৪ বা হ্যাং"]
  D --> E["nginx config debugging playbook"]
  E --> F["বাজেট মেলানো"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: কোন server block জিতল, regex location কেন match করল, রাত ২টায় $upstream_* variable কীভাবে পড়বেন।
...
কোন server block জিতল, regex location কেন match করল, রাত ২টায় $upstream_* variable কীভাবে পড়বেন। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
এজ ৪xx/৫xx, TLS হ্যান্ডশেক সময়, আর অরিজিন vs এজ হিট। **nginx config debugging playbook**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **nginx config debugging playbook**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: reverse-proxy-buffering-and-timeouts, load-balancing-algorithm-choice, nginx-edge-tls-termination।
