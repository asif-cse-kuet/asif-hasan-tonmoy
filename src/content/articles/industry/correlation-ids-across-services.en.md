> **Lesson 25 · beginner** - HTTP hops keep the request ID; the queue hop loses it, and the async half of the incident becomes invisible - propagate through headers, payload envelopes, and cron seeds.
...
- This lesson is specifically about **Correlation IDs across services and queues**. Tags: correlation-id, request-id, propagation, queues, laravel.
...
flowchart TD
  ...
  B --> C["Four log silos"]
  C --> D["Long TTD"]
  D --> E["Correlation IDs across services and queues"]
  E --> F["Correlate + SLI"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: HTTP hops keep the request ID; the queue hop loses it, and the async half of the incident becomes invisible - propagate through headers, payload envelopes, and cron seeds.
...
HTTP hops keep the request ID; the queue hop loses it, and the async half of the incident becomes invisible - propagate through headers, payload envelopes, and cron seeds. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
SLI burn, request-id join success, and alert pages that map to an owner. If the chart cannot catch a regression in **Correlation IDs across services and queues**, the lesson is not done.
...
- [ ] The invariant for **Correlation IDs across services and queues** is written in one sentence.
...
- [ ] Related reading still makes sense: distributed-tracing-adoption, structured-logging-standards, incident-timeline-reconstruction.
