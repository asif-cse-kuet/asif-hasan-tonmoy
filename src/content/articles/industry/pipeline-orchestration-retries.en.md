> **Lesson 107 · intermediate** - Task-level retries, sensor storms, and the difference between retrying a step and re-running a DAG run.
...
- This lesson is specifically about **Pipeline orchestration retries that do not amplify**. Tags: airflow, orchestration, retries, sla, dag.
...
flowchart TD
  ...
  D --> E["Pipeline orchestration retries that do not amplify"]
  E --> F["Contract + watermark"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Task-level retries, sensor storms, and the difference between retrying a step and re-running a DAG run.
...
Task-level retries, sensor storms, and the difference between retrying a step and re-running a DAG run. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Freshness, duplicate partition writes, and PII-scan findings. If the chart cannot catch a regression in **Pipeline orchestration retries that do not amplify**, the lesson is not done.
...
- [ ] The invariant for **Pipeline orchestration retries that do not amplify** is written in one sentence.
...
- [ ] Related reading still makes sense: idempotent-backfills, schema-drift-detection, batch-vs-streaming-pipelines.
