> **Lesson 155 · advanced** - Small requests work, large POSTs hang forever: PMTU black holes, tunnels, and the 1500-byte assumption nobody checks.
...
- This lesson is specifically about **MTU, MSS clamping, and silent packet loss**. Tags: mtu, mss, pmtud, tcp, packet-loss.
...
flowchart TD
  ...
  B --> C["Origin PHP"]
  C --> D["504 or hang"]
  D --> E["MTU, MSS clamping, and silent packet loss"]
  E --> F["Align budgets"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Small requests work, large POSTs hang forever: PMTU black holes, tunnels, and the 1500-byte assumption nobody checks.
...
Small requests work, large POSTs hang forever: PMTU black holes, tunnels, and the 1500-byte assumption nobody checks. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Edge 4xx/5xx, TLS handshake time, and origin vs edge cache hit. If the chart cannot catch a regression in **MTU, MSS clamping, and silent packet loss**, the lesson is not done.
...
- [ ] The invariant for **MTU, MSS clamping, and silent packet loss** is written in one sentence.
...
- [ ] Related reading still makes sense: tls-handshake-cost-and-resumption, websockets-through-proxies, nginx-config-debugging-playbook.
