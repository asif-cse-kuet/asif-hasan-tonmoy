> **পাঠ 72 · মাঝারি ধাপ** - deploy marker, alert timestamp ও log clock মিনিট-ব্যবধানে অমিল হলে post-incident review আটকে যায় - event annotate করুন, UTC-তে normalise করুন, raw evidence রাখুন।
...
- এই পাঠটা ঠিক **ঘটনার পরে incident timeline পুনর্গঠন** নিয়ে। ট্যাগ: postmortem, timeline, annotations, clock-skew, forensics।
...
flowchart TD
  ...
  B --> C["চার লগ সাইলো"]
  C --> D["দীর্ঘ TTD"]
  D --> E["ঘটনার পরে incident timeline পুনর্গঠন"]
  E --> F["কোরিলেট + SLI"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: deploy marker, alert timestamp ও log clock মিনিট-ব্যবধানে অমিল হলে post-incident review আটকে যায় - event annotate করুন, UTC-তে normalise করুন, raw evidence রাখুন।
...
deploy marker, alert timestamp ও log clock মিনিট-ব্যবধানে অমিল হলে post-incident review আটকে যায় - event annotate করুন, UTC-তে normalise করুন, raw evidence রাখুন। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
SLI বার্ন, রিকোয়েস্ট-আইডি জোড়া, আর মালিক-ম্যাপ করা অ্যালার্ট। **ঘটনার পরে incident timeline পুনর্গঠন**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **ঘটনার পরে incident timeline পুনর্গঠন**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: correlation-ids-across-services, structured-logging-standards, distributed-tracing-adoption।
