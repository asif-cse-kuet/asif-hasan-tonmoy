> **Lesson 102 · intermediate** - Closing the browser tab does not stop the meter - propagate abort signals from SSE all the way to the provider.
...
- This lesson is specifically about **Streaming UX, cancellation, and abandoned generations**. Tags: streaming, sse, cancellation, ttft, frontend.
...
flowchart TD
  ...
  D --> E["Streaming UX, cancellation, and abandoned generations"]
  E --> F["Eval + fallback"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Closing the browser tab does not stop the meter - propagate abort signals from SSE all the way to the provider.
...
Closing the browser tab does not stop the meter - propagate abort signals from SSE all the way to the provider. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Eval score, cost per successful answer, and empty-retrieval rate. If the chart cannot catch a regression in **Streaming UX, cancellation, and abandoned generations**, the lesson is not done.
...
- [ ] The invariant for **Streaming UX, cancellation, and abandoned generations** is written in one sentence.
...
- [ ] Related reading still makes sense: fallback-model-routing, multi-step-agent-loops-and-cost, llm-caching-and-cost-control.
