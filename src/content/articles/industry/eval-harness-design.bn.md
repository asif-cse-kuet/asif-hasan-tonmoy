> **পাঠ 167 · উন্নত ধাপ** - frozen golden set, retrieval-এ recall@k ও nDCG, generation-এ faithfulness, আর merge আটকানো threshold।
...
- এই পাঠটা ঠিক **CI-তে LLM feature-এর eval harness design** নিয়ে। ট্যাগ: evals, golden-set, ci, metrics, regression।
...
flowchart TD
  ...
  C --> D["ভুল উত্তর / খরচ"]
  D --> E["CI-তে LLM feature-এর eval harness design"]
  E --> F["ইভ্যাল + ফলব্যাক"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: frozen golden set, retrieval-এ recall@k ও nDCG, generation-এ faithfulness, আর merge আটকানো threshold।
...
frozen golden set, retrieval-এ recall@k ও nDCG, generation-এ faithfulness, আর merge আটকানো threshold। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
ইভ্যাল স্কোর, সফল উত্তরপ্রতি খরচ, আর খালি রিট্রিভাল হার। **CI-তে LLM feature-এর eval harness design**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **CI-তে LLM feature-এর eval harness design**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: rag-chunking-evals, hallucination-detection-and-citations, embedding-model-migration।
