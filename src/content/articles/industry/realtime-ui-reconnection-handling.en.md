> **Lesson 118 · advanced** - Backoff with jitter, resume cursors, and the snapshot fallback when the gap is too wide to replay.
...
- This lesson is specifically about **Realtime UI reconnection handling**. Tags: websocket, reconnect, realtime, resync, backoff.
...
flowchart TD
  ...
  D --> E["Realtime UI reconnection handling"]
  E --> F["Keyboard or LCP incident"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Backoff with jitter, resume cursors, and the snapshot fallback when the gap is too wide to replay.
...
Backoff with jitter, resume cursors, and the snapshot fallback when the gap is too wide to replay. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
LCP, JS transferred per route, and axe violations per deploy. If the chart cannot catch a regression in **Realtime UI reconnection handling**, the lesson is not done.
...
- [ ] The invariant for **Realtime UI reconnection handling** is written in one sentence.
...
- [ ] Related reading still makes sense: websocket-state-at-scale, offline-first-sync-conflicts.
