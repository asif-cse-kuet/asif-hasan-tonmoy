> **Lesson 38 · intermediate** - Byte and Core Web Vitals budgets enforced in CI, plus field RUM to catch what lab tests miss.
...
- This lesson is specifically about **Frontend performance budgets that hold**. Tags: performance, core-web-vitals, budgets, ci, rum.
...
flowchart TD
  ...
  D --> E["Frontend performance budgets that hold"]
  E --> F["Keyboard or LCP incident"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Byte and Core Web Vitals budgets enforced in CI, plus field RUM to catch what lab tests miss.
...
Byte and Core Web Vitals budgets enforced in CI, plus field RUM to catch what lab tests miss. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
LCP, JS transferred per route, and axe violations per deploy. If the chart cannot catch a regression in **Frontend performance budgets that hold**, the lesson is not done.
...
- [ ] The invariant for **Frontend performance budgets that hold** is written in one sentence.
...
- [ ] Related reading still makes sense: code-splitting-and-lazy-routes, rendering-strategy-selection.
