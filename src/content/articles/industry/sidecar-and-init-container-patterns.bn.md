> **পাঠ 152 · উন্নত ধাপ** - কখনো শেষ না হওয়া Job, proxy ready হওয়ার আগেই চালু হওয়া app, আর native sidecar যা দুটোই ঠিক করে।
...
- এই পাঠটা ঠিক **Sidecar ও init container pattern** নিয়ে। ট্যাগ: kubernetes, sidecar, init-containers, service-mesh, pods।
...
flowchart TD
  ...
  C --> D["রোলব্যাক তর্ক"]
  D --> E["Sidecar ও init container pattern"]
  E --> F["লেয়ার অর্ডার + প্রোব"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: কখনো শেষ না হওয়া Job, proxy ready হওয়ার আগেই চালু হওয়া app, আর native sidecar যা দুটোই ঠিক করে।
...
কখনো শেষ না হওয়া Job, proxy ready হওয়ার আগেই চালু হওয়া app, আর native sidecar যা দুটোই ঠিক করে। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
ইমেজ সাইজ, রোলআউট সময়, রিস্টার্ট সংখ্যা, আর রেডি হতে কতক্ষণ। **Sidecar ও init container pattern**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **Sidecar ও init container pattern**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: kubernetes-probes-done-right, docker-image-layer-optimization।
