> **পাঠ 134 · উন্নত ধাপ** - charge, email, webhook rollback হয় না - প্রতিটি external call dedup ও decision record দিয়ে ঘিরুন।
...
- এই পাঠটা ঠিক **At-least-once delivery ও বাস্তব side effect** নিয়ে। ট্যাগ: at-least-once, side-effects, payments, webhooks, stripe।
...
flowchart TD
  ...
  C --> D["ইউজার অভিযোগ"]
  D --> E["At-least-once delivery ও বাস্তব side effect"]
  E --> F["আইডেমপোটেন্ট কনজিউমার"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: charge, email, webhook rollback হয় না - প্রতিটি external call dedup ও decision record দিয়ে ঘিরুন।
...
charge, email, webhook rollback হয় না - প্রতিটি external call dedup ও decision record দিয়ে ঘিরুন। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
কিউ গভীরতা, ডেড-লেটার হার, আর ডুপ্লিকেট সাইড ইফেক্ট। **At-least-once delivery ও বাস্তব side effect**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **At-least-once delivery ও বাস্তব side effect**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: idempotent-consumers, exactly-once-delivery-illusion, saga-compensation-design।
