> **পাঠ 179 · উন্নত ধাপ** - checkpoint file মানেই version নয় - code, feature spec, label snapshot, আর এক command-এ ফেরার পথ লাগে।
...
- এই পাঠটা ঠিক **চাপের মধ্যে model versioning ও rollback** নিয়ে। ট্যাগ: mlops, versioning, rollback, registry, shadow-deploy।
...
flowchart TD
  ...
  D --> E["চাপের মধ্যে model versioning ও rollback"]
  E --> F["চুক্তি + ওয়াটারমার্ক"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: checkpoint file মানেই version নয় - code, feature spec, label snapshot, আর এক command-এ ফেরার পথ লাগে।
...
checkpoint file মানেই version নয় - code, feature spec, label snapshot, আর এক command-এ ফেরার পথ লাগে। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
ফ্রেশনেস, ডুপ্লিকেট পার্টিশন রাইট, আর PII স্ক্যান। **চাপের মধ্যে model versioning ও rollback**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **চাপের মধ্যে model versioning ও rollback**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: training-serving-skew, feature-store-consistency, imbalanced-classification-strategies।
