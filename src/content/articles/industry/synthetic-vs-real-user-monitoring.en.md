> **Lesson 76 · intermediate** - Your probe is green from one datacentre while a third of real users get 8-second loads - synthetics catch availability, RUM catches the experience.
...
- This lesson is specifically about **Synthetic checks vs real user monitoring**. Tags: synthetic-monitoring, rum, web-vitals, blackbox, slo.
...
flowchart TD
  ...
  B --> C["Four log silos"]
  C --> D["Long TTD"]
  D --> E["Synthetic checks vs real user monitoring"]
  E --> F["Correlate + SLI"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Your probe is green from one datacentre while a third of real users get 8-second loads - synthetics catch availability, RUM catches the experience.
...
Your probe is green from one datacentre while a third of real users get 8-second loads - synthetics catch availability, RUM catches the experience. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
SLI burn, request-id join success, and alert pages that map to an owner. If the chart cannot catch a regression in **Synthetic checks vs real user monitoring**, the lesson is not done.
...
- [ ] The invariant for **Synthetic checks vs real user monitoring** is written in one sentence.
...
- [ ] Related reading still makes sense: golden-signals-instrumentation, alert-design-and-noise-reduction, slo-error-budget-burn.
