> **পাঠ 28 · শুরুর ধাপ** - অধৈর্য user double-click করে, mobile network চুপচাপ retry করে, load balancer timed-out POST replay করে। Button disable করা UX; server-এ uniqueness constraint লাগে।
...
- এই পাঠটা ঠিক **Duplicate submission প্রতিরোধ** নিয়ে। ট্যাগ: idempotency, double-submit, dedup, forms, payments।
...
flowchart TD
  ...
  D --> E["Duplicate submission প্রতিরোধ"]
  E --> F["সীমা + ডিগ্রেড"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: অধৈর্য user double-click করে, mobile network চুপচাপ retry করে, load balancer timed-out POST replay করে। Button disable করা UX; server-এ uniqueness constraint লাগে।
...
অধৈর্য user double-click করে, mobile network চুপচাপ retry করে, load balancer timed-out POST replay করে। Button disable করা UX; server-এ uniqueness constraint লাগে। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
রিট্রাই অ্যামপ্লিফিকেশন, টাকা মিলের ফারাক, আর ডিপেন্ডেন্সি এরর বাজেট। **Duplicate submission প্রতিরোধ**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **Duplicate submission প্রতিরোধ**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: long-running-job-recovery, retry-storm-prevention, money-and-rounding-correctness।
