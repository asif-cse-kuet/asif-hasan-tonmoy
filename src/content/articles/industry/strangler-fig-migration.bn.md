> **পাঠ 184 · উন্নত ধাপ** - Facade স্থায়ী হয়ে গেলে আর দুই সিস্টেম একই write-এর মালিক হলে incremental replacement ব্যর্থ হয়। Sequencing, dual-read verification আর deletion deadline-ই পুরো কাজ।
...
- এই পাঠটা ঠিক **শেষ হয় এমন strangler fig migration** নিয়ে। ট্যাগ: migration, legacy, strangler-fig, rollout, dual-write।
...
flowchart TD
  ...
  D --> E["শেষ হয় এমন strangler fig migration"]
  E --> F["ADR + কিল সুইচ"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: Facade স্থায়ী হয়ে গেলে আর দুই সিস্টেম একই write-এর মালিক হলে incremental replacement ব্যর্থ হয়। Sequencing, dual-read verification আর deletion deadline-ই পুরো কাজ।
...
Facade স্থায়ী হয়ে গেলে আর দুই সিস্টেম একই write-এর মালিক হলে incremental replacement ব্যর্থ হয়। Sequencing, dual-read verification আর deletion deadline-ই পুরো কাজ। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
ফ্ল্যাগ এক্সপোজার %, প্ল্যাটফর্ম অ্যাডপশন, আর টেন্যান্টপ্রতি খরচ। **শেষ হয় এমন strangler fig migration**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **শেষ হয় এমন strangler fig migration**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: modular-monolith-vs-microservices, feature-flags-and-kill-switches, public-api-contract-stability।
