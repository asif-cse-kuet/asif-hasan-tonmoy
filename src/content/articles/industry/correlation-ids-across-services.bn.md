> **পাঠ 25 · শুরুর ধাপ** - HTTP hop request ID রাখে, queue hop হারায় - incident-এর async অর্ধেক অদৃশ্য হয়; header, payload envelope ও cron seed দিয়ে propagate করুন।
...
- এই পাঠটা ঠিক **service ও queue জুড়ে correlation ID** নিয়ে। ট্যাগ: correlation-id, request-id, propagation, queues, laravel।
...
flowchart TD
  ...
  B --> C["চার লগ সাইলো"]
  C --> D["দীর্ঘ TTD"]
  D --> E["service ও queue জুড়ে correlation ID"]
  E --> F["কোরিলেট + SLI"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: HTTP hop request ID রাখে, queue hop হারায় - incident-এর async অর্ধেক অদৃশ্য হয়; header, payload envelope ও cron seed দিয়ে propagate করুন।
...
HTTP hop request ID রাখে, queue hop হারায় - incident-এর async অর্ধেক অদৃশ্য হয়; header, payload envelope ও cron seed দিয়ে propagate করুন। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
SLI বার্ন, রিকোয়েস্ট-আইডি জোড়া, আর মালিক-ম্যাপ করা অ্যালার্ট। **service ও queue জুড়ে correlation ID**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **service ও queue জুড়ে correlation ID**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: distributed-tracing-adoption, structured-logging-standards, incident-timeline-reconstruction।
