> **Lesson 180 · advanced** - Feature contracts, backfills, and silent metric cliffs after deploy.
...
- This lesson is specifically about **Training–serving skew in ML pipelines**. Tags: mlops, feature-store, pipelines.
...
flowchart TD
  ...
  D --> E["Training–serving skew in ML pipelines"]
  E --> F["Contract + watermark"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Feature contracts, backfills, and silent metric cliffs after deploy.
...
Feature contracts, backfills, and silent metric cliffs after deploy. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Freshness, duplicate partition writes, and PII-scan findings. If the chart cannot catch a regression in **Training–serving skew in ML pipelines**, the lesson is not done.
...
- [ ] The invariant for **Training–serving skew in ML pipelines** is written in one sentence.
