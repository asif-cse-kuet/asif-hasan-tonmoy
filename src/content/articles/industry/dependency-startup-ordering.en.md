> **Lesson 77 · intermediate** - Services that crash-loop because Postgres is not up yet turn a 30-second database restart into a 20-minute cluster-wide outage. Startup order is a runtime property, not a deploy script.
...
- This lesson is specifically about **Dependency startup ordering**. Tags: startup, health-checks, probes, bootstrap, kubernetes.
...
The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Services that crash-loop because Postgres is not up yet turn a 30-second database restart into a 20-minute cluster-wide outage. Startup order is a runtime property, not a deploy script.
...
Services that crash-loop because Postgres is not up yet turn a 30-second database restart into a 20-minute cluster-wide outage. Startup order is a runtime property, not a deploy script. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Retry amplification, money-reconciliation diffs, and dependency error budget. If the chart cannot catch a regression in **Dependency startup ordering**, the lesson is not done.
...
- [ ] The invariant for **Dependency startup ordering** is written in one sentence.
...
- [ ] Related reading still makes sense: chaos-experiments-safely, graceful-degradation-design, retry-storm-prevention.
