> **Lesson 28 · beginner** - Impatient users double-click, mobile networks retry silently, and load balancers replay timed-out POSTs. Disabling the button is a UX nicety; the server needs a uniqueness constraint.
...
- This lesson is specifically about **Duplicate submission prevention**. Tags: idempotency, double-submit, dedup, forms, payments.
...
flowchart TD
  ...
  B --> C["Worse failure"]
  C --> D["Finance ticket"]
  D --> E["Duplicate submission prevention"]
  E --> F["Bound + degrade"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Impatient users double-click, mobile networks retry silently, and load balancers replay timed-out POSTs. Disabling the button is a UX nicety; the server needs a uniqueness constraint.
...
Impatient users double-click, mobile networks retry silently, and load balancers replay timed-out POSTs. Disabling the button is a UX nicety; the server needs a uniqueness constraint. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Retry amplification, money-reconciliation diffs, and dependency error budget. If the chart cannot catch a regression in **Duplicate submission prevention**, the lesson is not done.
...
- [ ] The invariant for **Duplicate submission prevention** is written in one sentence.
...
- [ ] Related reading still makes sense: long-running-job-recovery, retry-storm-prevention, money-and-rounding-correctness.
