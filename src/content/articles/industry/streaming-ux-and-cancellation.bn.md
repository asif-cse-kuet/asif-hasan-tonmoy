> **পাঠ 102 · মাঝারি ধাপ** - browser tab বন্ধ করলে মিটার থামে না - SSE থেকে provider পর্যন্ত abort signal propagate করুন।
...
- এই পাঠটা ঠিক **Streaming UX, cancellation ও পরিত্যক্ত generation** নিয়ে। ট্যাগ: streaming, sse, cancellation, ttft, frontend।
...
flowchart TD
  ...
  C --> D["ভুল উত্তর / খরচ"]
  D --> E["Streaming UX, cancellation ও পরিত্যক্ত generation"]
  E --> F["ইভ্যাল + ফলব্যাক"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: browser tab বন্ধ করলে মিটার থামে না - SSE থেকে provider পর্যন্ত abort signal propagate করুন।
...
browser tab বন্ধ করলে মিটার থামে না - SSE থেকে provider পর্যন্ত abort signal propagate করুন। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
ইভ্যাল স্কোর, সফল উত্তরপ্রতি খরচ, আর খালি রিট্রিভাল হার। **Streaming UX, cancellation ও পরিত্যক্ত generation**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **Streaming UX, cancellation ও পরিত্যক্ত generation**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: fallback-model-routing, multi-step-agent-loops-and-cost, llm-caching-and-cost-control।
