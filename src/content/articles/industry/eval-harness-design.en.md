> **Lesson 167 · advanced** - Frozen golden sets, recall@k and nDCG for retrieval, faithfulness for generation, and thresholds that block a merge.
...
- This lesson is specifically about **Eval harness design for LLM features in CI**. Tags: evals, golden-set, ci, metrics, regression.
...
flowchart TD
  ...
  D --> E["Eval harness design for LLM features in CI"]
  E --> F["Eval + fallback"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Frozen golden sets, recall@k and nDCG for retrieval, faithfulness for generation, and thresholds that block a merge.
...
Frozen golden sets, recall@k and nDCG for retrieval, faithfulness for generation, and thresholds that block a merge. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Eval score, cost per successful answer, and empty-retrieval rate. If the chart cannot catch a regression in **Eval harness design for LLM features in CI**, the lesson is not done.
...
- [ ] The invariant for **Eval harness design for LLM features in CI** is written in one sentence.
...
- [ ] Related reading still makes sense: rag-chunking-evals, hallucination-detection-and-citations, embedding-model-migration.
