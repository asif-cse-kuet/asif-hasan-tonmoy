> **Lesson 103 · intermediate** - Freshness requirements, not fashion, decide the engine - and most teams need one streaming path plus a batch reconciler.
...
- This lesson is specifically about **Batch vs streaming: picking the cheaper correctness**. Tags: batch, streaming, kafka, freshness, architecture.
...
flowchart TD
  ...
  D --> E["Batch vs streaming: picking the cheaper correctness"]
  E --> F["Contract + watermark"]
```

The named technology is rarely the villain. Two code paths (often a Vue/Quasar screen and a Laravel write) assume opposite contracts. Production finds the race. For this topic that contract is: Freshness requirements, not fashion, decide the engine - and most teams need one streaming path plus a batch reconciler.
...
Freshness requirements, not fashion, decide the engine - and most teams need one streaming path plus a batch reconciler. Put that sentence in the PR, the Pinia action, and the Laravel class.
...
Freshness, duplicate partition writes, and PII-scan findings. If the chart cannot catch a regression in **Batch vs streaming: picking the cheaper correctness**, the lesson is not done.
...
- [ ] The invariant for **Batch vs streaming: picking the cheaper correctness** is written in one sentence.
...
- [ ] Related reading still makes sense: late-arriving-out-of-order-data, etl-vs-elt-decisions, pipeline-orchestration-retries.
