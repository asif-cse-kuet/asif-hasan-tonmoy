> **পাঠ 135 · উন্নত ধাপ** - compatibility mode, required-field ফাঁদ, ছয়টা consumer নিজের schedule-এ deploy করলে producer change rollout।
...
- এই পাঠটা ঠিক **Consumer না ভেঙে event schema evolution** নিয়ে। ট্যাগ: schema-registry, avro, compatibility, contracts, protobuf।
...
flowchart TD
  ...
  C --> D["ইউজার অভিযোগ"]
  D --> E["Consumer না ভেঙে event schema evolution"]
  E --> F["আইডেমপোটেন্ট কনজিউমার"]
```

ভাঙে প্রযুক্তির নামে নয়, চুক্তির অভাবে। Vue/Quasar স্ক্রিন আর Laravel রাইট যদি উল্টো উত্তর ধরে, প্রোডাকশন সেই রেসটা খুঁজে বের করে। এই টপিকের চুক্তিটা হলো: compatibility mode, required-field ফাঁদ, ছয়টা consumer নিজের schedule-এ deploy করলে producer change rollout।
...
compatibility mode, required-field ফাঁদ, ছয়টা consumer নিজের schedule-এ deploy করলে producer change rollout। এই বাক্য পিআর, Pinia অ্যাকশন, আর Laravel ক্লাসে রাখুন।
...
কিউ গভীরতা, ডেড-লেটার হার, আর ডুপ্লিকেট সাইড ইফেক্ট। **Consumer না ভেঙে event schema evolution**-এর রিগ্রেশন এই চার্টে না ধরা পড়লে পাঠ শেষ হয়নি।
...
- [ ] **Consumer না ভেঙে event schema evolution**-এর ইনভেরিয়েন্ট এক বাক্যে লেখা।
- [ ] খালি, ডুপ্লিকেট, টাইমআউট বা পারমিশন পাথের টেস্ট বা রিহার্সাল আছে।
- [ ] এক ডিপ্লয়ের মধ্যে রিগ্রেশন ধরার লগ বা চার্ট আছে।
- [ ] আত্মীয় পাঠ এখনও মিলছে: poison-message-and-dlq, queue-vs-stream-selection, outbox-pattern-implementation।
